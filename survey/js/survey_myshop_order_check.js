  ...
  async function (event) {
    (async function (CAFE24API) {
      setTimeout(async function () {
        ...
        let mode = surveyCommon.shoplusGetParameters("mode");

        let _order_id = surveyCommon.shoplusGetParameters("order_id");
        ...
        let survey_diagnosis_result = await surveyOrderList.getSurveyResult(history_start_date, history_end_date);


        let status_type = "N00,N10,N22,N20,N21,N30,N40,N50,C00,C10,C34,C36,C40,C47,C48,C49,R00,R10,R12,R13,R30,R34,R36,R40";
        // 처방 분석 조회 (상품준비중: N10)
        if (status == "analysis") {
          status_type = "N10,N22";
        }
        ...
        for (const order of survey_order_list) {
          ...
          for (const item of order.items) {
            ...
            if (find) {
              ...
              if (find.before_hash == "") {
                view_result_url = `/${view_path}/${resilt_file}?product_no=${find_product_no}&hash=${product_hash_code}&qna_at=${find_qna_at}`
              } else {
              ...
              if (find.before_hash == "") {
                if (find.discount_type && find.discount_type.indexOf("employees") > -1) {
                  ...
                  if (find.discount_type == "employees1") {
                    survey_btn_rebuy_style = "display: none";
                    let _find_product = surveyCommon.survey_product_list[surveyCommon.survey_mall_id].find((e) => Number(e.no) == Number(find_product_no));
                    let product_type = _find_product.product_type.replace("employees_", "");
                    discount_type += "&buy_type=" + product_type;
                  }
            ...
            <li class="diagnosis_order survey_order_area displaynone">
                <div class="top">
                    ...
                    <a href="/myshop/order/detail.html?order_id=${order_id}&page=1&history_start_date=${order_date}&history_end_date=${order_date}${discount_type}" class="moreView"></a>
                </div>
                ${gift_html}
                <div class="mid">
                    <div class="prdOrder">
                        <div class="info">
                            ...
                            <span class="price">${surveyCommon.surveyComma(Number(payment_amount))}원 / <em>${product_quantity}개</em></span>
                            <div class="state">
                        ...
                        <div class="thumbnail">
                            <a href="${view_result_url}"><img src="${product_image}"></a>
                        </div>
                    </div>
                </div>
                <div class="mBtn gColumn">
                  ...
                  <a
                    href="#none"
                    class="btnItemFunc survey_cancel ${sample_hide}"
                ...
                <div class="mBtn gColumn ${nick_name_display}">
                  ...
                  <a
                    href="#none"
                    class="btnItemFunc black feedback btnRebuy ${single_item_hide}"
...
function surveySearch() {
  ...
  location.href = `/myshop/mypage/order_check.html?history_start_date=${history_start_date}&history_end_date=${history_end_date}${status}`;
}
...
async function getSurveyOrderListDiagnosis () {
  ...
  return new Promise(function (resolve, reject) {
    survey_jQuery.ajax({
      url: `${surveyCommon.survey_domain}/app/${surveyCommon.survey_app_name}/mall/${surveyCommon.survey_mall_id}/api/survey/shops/${surveyCommon.survey_shop_no}/front/diagnosis/member`,
      type: "get",
      data: set_param,
      dataType: "json",
