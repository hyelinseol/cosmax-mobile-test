...
async function setSurveyMoreResultDiagnosis(params) {
  ...
  // BOM_CODE 입력
  survey_jQuery(survey_target + " .survey_bom_code").text(params.bom_code[0].substr(survey_bom_cut_start, survey_bom_cut_end));
  ...
  // 선택한 고민 노출
  await setSurveyWorrySelect(survey_target);
  ...
  // sub_survey_target
  setSurveySubDiagnosis(sub_survey_target, befor_result, survey_befor_worry_array);
  ...
  for (const ampoule of params.osdl_result.point2.ampoules) {
    ...
    if (find_befor_ampoule) {
      ...
      if (find_param_ampoule) {
        ...
        if (find_param_ampoule.ratio == find_befor_ampoule.ratio) {
          icon_html = '<div class="icoArea icodown"><strong class="keep">유지</strong></div>';
        }
        if (find_param_ampoule.ratio < find_befor_ampoule.ratio) {
...
function setSurveyScent(survey_target, params, befor_bom = null) {
  console.log(survey_target, params.osdl_result.point3.scent.name);
  survey_jQuery(survey_target + " .survey_scent_type").parent().show();
  survey_jQuery(survey_target + " .survey_scent_top").parent().parent().show();
  if (!params.osdl_result.point3.scent.name) {
    ...
    survey_jQuery(survey_target + " .survey_scent_type").parent().hide();
    survey_jQuery(survey_target + " .survey_scent_top").parent().parent().hide();
  }
  ...
  console.log("survey_target", survey_target);
...
function setSurveyOSDL(params, survey_target) {
  // ODSL 이미지 하단 텍스트
  survey_jQuery(survey_target + " #survey_osdl_text_top").html(surveyEnterBr(params.osdl_result.osdl.mobile_text.top));
  ...
  if (params.osdl_result.osdl.scalp == "O") {
    survey_jQuery(survey_target + " #survey_scalp_type").html('<strong class="">지성</strong>이라면?');
    scalp_list = params.osdl_result.point4.hair_tip.scalp;
    survey_scalp_type_image = "/web/upload/mynomy/kr_mobile/prd/scalp01.png";
  } else if (params.osdl_result.osdl.scalp == "N") {
    survey_jQuery(survey_target + " #survey_scalp_type").html('<strong class="">중성</strong>이라면?');
    scalp_list = params.osdl_result.point4.hair_tip.scalp;
    survey_scalp_type_image = "/web/upload/mynomy/kr_mobile/prd/scalp02.png";
  } else {
    survey_jQuery(survey_target + " #survey_scalp_type").html('<strong class="">건성</strong>이라면?');
    scalp_list = params.osdl_result.point4.hair_tip.scalp;
    survey_scalp_type_image = "/web/upload/mynomy/kr_mobile/prd/scalp03.png";
  }

  survey_jQuery(survey_target + " #survey_scalp_type").nextAll().remove();
  ...
  survey_jQuery(survey_target + " #survey_sensitive_type").nextAll().remove();
  for (const sensitive of sensitive_list) {
    ...
    survey_jQuery(survey_target + " #survey_sensitive_type_image").attr("src", survey_sensitive_type_image);
  }
  ...
  if (params.osdl_result.osdl.hair == "H") {
    survey_jQuery(survey_target + " #survey_hair_type").html('<strong class="">건강모</strong>라면?');
    hair_list = params.osdl_result.point4.hair_tip.hair;
    survey_hair_type_image = "/web/upload/mynomy/kr_mobile/prd/scalp06.png";
  } else if (params.osdl_result.osdl.hair == "D") {
    survey_jQuery(survey_target + " #survey_hair_type").html('<strong class="">손상모</strong>라면?');
    hair_list = params.osdl_result.point4.hair_tip.hair;
    survey_hair_type_image = "/web/upload/mynomy/kr_mobile/prd/scalp07.png";
  } else {
    survey_jQuery(survey_target + " #survey_hair_type").html('<strong class="">(극)손상모</strong>라면?');
    hair_list = params.osdl_result.point4.hair_tip.hair;
    survey_hair_type_image = "/web/upload/mynomy/kr_mobile/prd/scalp07.png";
  }

  survey_jQuery(survey_target + " #survey_hair_type").nextAll().remove();
  ...
  if (params.osdl_result.osdl.hair_loss == "L") {
    survey_jQuery(survey_target + " #survey_hair_loss_type").html('<strong class="">탈모</strong>라면?');
  ...
  survey_jQuery(survey_target + " #survey_hair_loss_type").nextAll().remove();
  for (const hair_loss of hair_loss_list) {
    ...
    survey_jQuery(survey_target + " #survey_hair_loss_type_image").attr("src", survey_hair_loss_type_image);
  }
}
...
function setSurveySubDiagnosis(sub_survey_target, params, survey_worry_array) {
  ...
  */
  ...
  for (const worry of survey_worry_array) {
    survey_jQuery(sub_survey_target + " .survey_worry_list").append(`<li><span class="check"></span><span class="word">${worry}</span></li>`);
  }
  /* 고객님을 위한 솔루션 콤플렉스*/
  ...
  for (let ampoule of params.osdl_result.point2.ampoules) {
    ...
    if (ampoule_name == "돌콩배아추출물") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple01.gif";
    }
    if (ampoule_name == "딥클린 콤플렉스™" || ampoule_name == "딥클린 콤플렉스 ™") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple02.gif";
    }
    if (ampoule_name == "피록톤올아민") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple03.gif";
    }
    if (ampoule_name == "멘톨&허브쿨" || ampoule_name == "멘톨 & 허브쿨") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple04.gif";
    }
    if (ampoule_name == "세범컨트롤 콤플렉스™" || ampoule_name == "세범컨트롤 콤플렉스 ™") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple05.gif";
    }
    if (ampoule_name == "자라나리옴") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple06.gif";
    }
    if (ampoule_name == "미라클미네랄") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple07.gif";
    }
    if (ampoule_name == "판테놀&아쿠아씰" || ampoule_name == "판테놀 & 아쿠아씰") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple08.gif";
    }
    if (ampoule_name == "프로텍션 콤플렉스™" || ampoule_name == "프로텍션 콤플렉스 ™") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple09.gif";
    }
    if (
      ...
      ampoule_image = "/web/upload/mynomy/kr/prd/emple10.gif";
    }
    if (ampoule_name == "엔젤링 콤플렉스™" || ampoule_name == "엔젤링 콤플렉스 ™") {
      ampoule_image = "/web/upload/mynomy/kr/prd/emple11.gif";
    }
...
async function getSurveyBoms() {
  ...
  for (const bom_code of survey_re_qna_info.bom_code) {
    ...
    if (bom_code.indexOf("CHT") > -1) {
      survey_ingredient_text_list.push("<span style='font-weight: bold;'>트리트먼트</span> : " + survey_ingredients.join(", "));
    }
    if (bom_code.indexOf("CSP") > -1) {
      survey_ingredient_text_list.unshift("<span style='font-weight: bold;'>샴푸</span> : " + survey_ingredients.join(", "));
    }
  ...
  async function (event) {
    setTimeout(async function () {
      ...
      setTimeout(async function () {
        ...
        if (survey_product_no == survey_set_product_no) {
          survey_product_resource_info.retail_price = Number(survey_product_resource_info.retail_price) + Number(survey_product_resource.treatment.retail_price);
          survey_product_resource_info.discountprice.mobile_discount_price = Number(survey_product_resource_info.discountprice.mobile_discount_price) + Number(survey_product_resource.treatment.discountprice.mobile_discount_price);
          survey_product_resource_info.price = Number(survey_product_resource_info.price) + Number(survey_product_resource.treatment.price);
        }


        let survey_retail_price = Number(survey_product_resource_info.retail_price);
        let survey_discount_price = Number(survey_product_resource_info.discountprice.mobile_discount_price);
        ...
        // 문진 bom 조회
        await getSurveyBoms();
        let survey_re_shampoo_bom_code = survey_jQuery("#survey_re_shampoo_area .survey_bom_code").text();
        if (survey_re_shampoo_bom_code && survey_re_shampoo_bom_code[0] == "9") {
          survey_jQuery(".survey_is_hairless").text("탈모개선");
        }
