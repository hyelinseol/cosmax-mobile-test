...
async function getSurveyResultDiagnosis() {
  ...
  return new Promise(function (resolve, reject) {
    survey_jQuery.ajax({
      ...
      success: async function (result) {
        ...
        if (result && result[0] && result[0].product_id) {
        ...
        survey_diagnosis_result_list = result[0];
        resolve(result[0]);
      },
      error: function (request, status, error) {
        resolve([]);
...
function surveyProductSelect(type) {
  ...
  // survey_jQuery(".box-right .product-info .shadow").attr("style", "");
  // survey_jQuery("#survey_shadow").css("position", "");
  ...
  async function (event) {
    setTimeout(async function () {
      ...
      if (hash) {
        ...
        survey_result_diagnosis = await getSurveyResultDiagnosis();
      } else {
