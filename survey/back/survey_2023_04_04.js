...
let survey_hair_descript = [
  {
    subject: "건강모",
    content: "손빗질시 엉킴이<br>전혀 없고<br>윤기있는 모발이며,<br>염색 이력 없음",
  },
  { subject: "손상모 1단계", content: "손빗질시 엉킴이<br>거의 없음" },
  { subject: "손상모 2단계", content: "손빗질시 엉킴이<br>있긴하지만<br>쉽게 풀어지는 편" },
  { subject: "손상모 3단계", content: "한 두번의<br>손빗질로는<br>쉽게 풀리지 않고<br>엉킴" },
  {
    subject: "극손상모",
    content: "엉킴이 심해<br>손빗질이 어렵고<br>모발 끝이<br>갈라진 모발이 많음",
  },
];
...
function setSurveyAnswer(
  ...
  if (answer_type == "text") {
    ...
    let fix_asnwer_html = `
      <input type="radio" name="strength" class="radiobox" onClick="surveyRadioClick(this)" checked value="normal"><label>보통</label>
      <input type="radio" name="strength" class="radiobox" onClick="surveyRadioClick(this)" value="strong"><label>강하게</label>
    `;
    answer_html += `
            </div>
          ...
          if (j == folder_sequence) {
            ...
            if (question_subject.indexOf("<br>") > -1) {
              ...
              if (subject_array.length > 0 && subject_array[0] == "나의 곱슬 정도는?") {
                ...
                subject_array.shift();
                bottom_subject = "<p>" + subject_array.join("") + "</p>";
              }
            }
...
function surveyAnswerPresetClick(element) {
  ...
  let subject = survey_jQuery(".contentBox .txt h3").text().trim();
