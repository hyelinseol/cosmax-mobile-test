...
async function surveyResultCart(type) {
  ...
  for (const product of product_list) {
    ...
    await surveySaveCart(product_no, bom_code, nick_name, qna_type, hash, quantity);
/*
...
async function surveySaveCart(
  ...
  let variant_code = "";
  let shampoo_bom = null;
  let treatment_bom = null;
  ...
  if (Number(product_no) !== Number(survey_set_product_no)) {
    ...
    let bom = "";
    if (Number(product_no) == Number(survey_shampoo_no)) {
      variant_code = survey_variant_resource.shampoo.variant_code;
      bom = shampoo_bom;
    }
    if (Number(product_no) == Number(survey_treatment_no)) {
      variant_code = survey_variant_resource.treatment.variant_code;
      bom = treatment_bom;
    }
            ...
            additional_option_values: [
              {
                key: "item_option_add",
                type: "text",
                name: option_result.options.additional_options[0].additional_option_name,
                value: bom_code,
              },
              {
                key: "item_option_add",
                type: "text",
                name: option_result.options.additional_options[1].additional_option_name,
                value: nick_name,
              },
              {
                key: "item_option_add",
                type: "text",
                name: option_result.options.additional_options[2].additional_option_name,
                value: hash,
              },
            ],
          },
